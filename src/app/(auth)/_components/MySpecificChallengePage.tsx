...
export default function MySpecificChallengePage() {
  const { id } = useParams();
  ...
  return (
    <div className="mt-24 flex w-996 flex-col gap-16">
      <div
        className={`flex h-35 w-full items-center justify-center text-16 font-semibold ${chipBackgroundColor} ${chipTextColor}`}
      >
        {challengeStatus === 'WAITING' && '승인 대기 중입니다.'}
        {challengeStatus === 'REFUSE' && '선청이 거절되었습니다.'}
      </div>
